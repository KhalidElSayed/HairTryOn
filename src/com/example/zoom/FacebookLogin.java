...
public class FacebookLogin extends Activity {
	...
	String userdata,reqData,emailvalidate,passwordaboutuser,errormessages,stremail,strpassword;
	Context context;
	AppConfig config;
    ...
    public void onCreate(Bundle savedInstanceState) {
       ...
       /* SharedPreferences settings = getSharedPreferences(PREFS_NAME, 0);
		if (settings.getString("logged", "").toString().equals("logged")) {
			Intent intent = new Intent(FacebookLogin.this, Capture.class);
        ...
        login.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				/*if(edtemail.getText().toString().length() > 0 && edtpassword.getText().toString().length() > 0 ) {
					if(edtemail.getText().toString().equals(stremail= edtemail.getText().toString()) && edtpassword.getText().toString().equals(strpassword= edtpassword.getText().toString())) {
...
//		});
    ...
    public class performBackgroundTask extends AsyncTask<Void, Void, Void> {
		...
		private void SaveUserData() {
			...
			String signupUrl = "http://webplanex.net/projects/donna_bella/iphone_api/api.php?task=login&format=json&data[email]="+stremail+"&data[password]="+strpassword+"";
    ...
    private final class FbLoginDialogListener implements DialogListener {
        ...
        public void onFacebookError(FacebookError error) {
           Toast.makeText(FacebookLogin.this, "Facebook connection failed", Toast.LENGTH_SHORT).show();
	...
	private Handler mFbHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			if (msg.what == 0) {
		        ...
		        Toast.makeText(FacebookLogin.this, "Connected to Facebook as " + username, Toast.LENGTH_SHORT).show();
		        
		        config.facebookname = username;
		        Intent fblogin = new Intent(getApplicationContext(),Capture.class);
		        startActivity(fblogin);
		        finish();
			} else {
				Toast.makeText(FacebookLogin.this, "Connected to Facebook", Toast.LENGTH_SHORT).show();
			}
		}
	};
	...
	private Handler mHandler = new Handler() 
	{
		...
		public void handleMessage(Message msg) 
		{
			mProgress.dismiss();
			if (msg.what == 1) {
				Toast.makeText(FacebookLogin.this, "Facebook logout failed", Toast.LENGTH_SHORT).show();
			} else {
				mFacebookBtn.setChecked(false);
	        	mFacebookBtn.setText("  Facebook (Not connected)");
	        	mFacebookBtn.setTextColor(Color.GRAY);
	        	   
				Toast.makeText(FacebookLogin.this, "Disconnected from Facebook", Toast.LENGTH_SHORT).show();
			}
		}
	};
}